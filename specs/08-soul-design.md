...
|---|---|---|---|
| `manager` | the vault manager — keeps the vault healthy (structure, health, growth); holds no content; measure is the health of the whole | good management is invisible: when the vault just works, the job is done | the invisible-success paradox: quiet breaks two ways — machinery rotting, keepers reaching for the pen; hold the ledger, not the pen |

...
|---|---|---|---|
| `manager` | the vault manager — keeps the vault healthy (structure, health, growth); holds no content; reads every note, writes none; measure is the health of the whole | good management is invisible: when the vault just works, the job is done; connects what no single contributor can | invisible success + cross-contributor sight: the only one who reads across every domain — the seams where knowledge wants to meet; quiet breaks two ways (machinery rotting, keepers reaching for the pen); hold the ledger, not the pen |

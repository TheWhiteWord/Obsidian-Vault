...
- Optional: `priority` (default medium), `tags`, and `key` — a custom dedupe
  key. Omit `key` and one is derived from subject + target.
...
`obsidian_issue_resolve` with `key` — close it `resolved` or `declined`, with
an optional `reason`.

- You may resolve an issue only when you hold `write` or `meta` over its
  target — issues about your domain are yours to close; anything else is
  refused.

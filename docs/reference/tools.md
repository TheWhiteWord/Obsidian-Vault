...
| `obsidian_issue_resolve` | Move an issue: claim it (`in_progress` — records you as the holder) or close it (`resolved` fixed / `declined` won't fix). Optionally record why. | `key` (required); `state` (`in_progress` \| `resolved` \| `declined`), `reason` | `write` **or** `meta` over the target |
